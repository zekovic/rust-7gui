...
https://github.com/fltk-rs/fltk-table/blob/main/src/lib.rs
...
use fltk::{
    app::{self, event_original_key}, draw,
    enums::*,
    input,
...
impl SmartTable {
    ...
    pub fn set_opts(&mut self, opts: TableOpts) {
        ...
        if opts.editable {
            ...
            //inp.set_frame(FrameType::BorderBox);
            inp.hide();
            ...
            self.table.handle({
                let data = self.data.clone();
                move |t, ev| match ev {
                    Event::Released | Event::KeyDown => {
                        // add option to open input by keyboard
                        if ev == Event::KeyDown 
                                && !app::event_key_down(fltk::enums::Key::F2)
                                && !app::event_key_down(fltk::enums::Key::Tab)
                                && !app::event_key_down(fltk::enums::Key::Enter) {
                            return false;
                        }
                        // prevent opening input on selecting multiple cells
                        let range = t.get_selection();
                        ...
                        if let Ok(data) = data.try_lock() {
                            ...
                            inp.resize(cell.x-1, cell.y-1, cell.w+1, cell.h+1); // better input size when cell border is 1px
    ...
    fn draw_data(txt: &str, x: i32, y: i32, w: i32, h: i32, selected: bool, opts: &TableOpts) {
        ...
        draw::draw_rect(x-1, y-1, w+1, h+1);  // fix line thickness
        draw::pop_clip();
    }
